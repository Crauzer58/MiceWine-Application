...
class WelcomeActivity : AppCompatActivity() {
    ...
    private val fragmentList: List<Fragment> = listOf(
        WelcomeFragment(R.layout.fragment_welcome),
        WelcomeFragment(R.layout.fragment_welcome_2)
    )
    private var selectedFragment = 0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        button?.setOnClickListener {
            ...
            when (selectedFragment) {
                2 -> {
                    if (appBuiltinRootfs) {
                        sendBroadcast(Intent(ACTION_SETUP))
                    }
                    finish()
                }
            }
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent?): Boolean {
        if (keyCode == KeyEvent.KEYCODE_BACK && selectedFragment == 0) {
            return true
        }
        return super.onKeyDown(keyCode, event)
    }
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        if (requestCode == REQUEST_CODE) {
            fileManagementPermission = grantResults.isNotEmpty() &&
    ...
    override fun onResume() {
        super.onResume()
        fileManagementPermission = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
